var App = (function (app) {
    ...
    function updatePlaylist(mixes) {
        var list = $('#playlist > ul'),
            item = '<li><a href="{{url}}" alt="{{desc}}" title="{{desc}}" class="icon-">{{name}}</a></li>\n'
        ;
        
        list.html('');
		var firstMix = mixes.shift();
        mixes.forEach(function(mix) {
		
            list.append(item.replace('{{url}}', mix.url).replace('{{desc}}', mix.desc).replace('{{desc}}', mix.desc).replace('{{name}}', mix.name));
        });
		
		list.prepend(item.replace('{{url}}', firstMix.url).replace('{{desc}}', firstMix.desc).replace('{{desc}}', firstMix.desc).replace("icon-","icon-music").replace('{{name}}', " "+firstMix.name));
